...
# Introduction
Marathon performance is influenced by numerous factors, with environmental conditions such as temperature and humidity playing a significant role. Higher temperatures are known to negatively impact endurance, which is particularly critical in long-distance events like marathons. Older runners often encounter greater difficulties in managing heat, which increases their susceptibility to heat-related challenges. Additionally, gender differences contribute to varying physiological responses, further shaping performance outcomes under diverse environmental stresses.
...
The AQI dataset provided average daily measurements for air quality during marathon events. However, these values may represent varying measurement methods across different marathons. To ensure the validity of using these measurements in the analysis, further investigation into the consistency and methodology of AQI reporting across locations is needed. This could involve aligning the data with additional environmental reports or justifying the aggregation method used in this study.
...
table_data <- df_table[, c(2, 3)]

kable(table_data, format = "latex", booktabs = TRUE, caption = "Summary of Marathon Data") %>%
...
```{r fig.width=7, fig.height=5, echo=FALSE, fig.align='center', warning=FALSE, message=FALSE}
...
The analysis of age effects on marathon performance highlights a clear non-linear relationship between age and performance across both genders. Performance peaks in the late 20s to early 30s, with men and women achieving mean performance deviations from the course record (%CR) of approximately 36% (SD = 10%) and 40% (SD = 8%), respectively. After this peak, a gradual decline is observed, with older adults (50+ years) showing a mean performance deviation of 50% (SD = 12%). Men exhibit greater variability at older ages, as indicated by wider confidence intervals, while women demonstrate a more consistent decline. These findings emphasize the significant role of age and gender in shaping marathon outcomes.
...
To align with the findings in Aim 1, we categorized age into four groups (e.g., 20-29, 30-39, 40-49, 50+) based on performance trends identified in the previous analysis. This grouping reflects distinct physiological characteristics and performance patterns, enabling a more consistent comparison across aims.
...
## sumamry of aim 2
The analysis of environmental conditions on marathon performance reveals a significant interaction between gender, age, and weather parameters. Specifically, younger runners (20–29 years) demonstrated better adaptability to elevated Wet Bulb Globe Temperatures (WBGT) compared to older groups (50+ years), whose performance showed a marked decline under similar conditions. For example, the %CR deviation increased by approximately 10% for runners in the oldest category when WBGT exceeded 23°C.

Additionally, gender-specific differences were observed. Male runners exhibited a steeper decline in performance as WBGT and relative humidity increased, particularly in the 40–49 age group. In contrast, female runners demonstrated greater resilience, with relatively consistent performance trends across age groups under moderate environmental stress. However, extreme conditions (WBGT > 25°C) impacted both genders significantly, with female runners in the 30–39 age group showing a 7% greater performance deviation compared to their male counterparts.

These findings underscore the importance of considering demographic and environmental interactions when evaluating marathon performance. The results also suggest the need for tailored strategies to mitigate the impact of adverse weather conditions, particularly for older and male runners.
